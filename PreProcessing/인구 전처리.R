...
data.all$읍면동명 <- strsplit(data.all$읍면동명,split='구') %>% sapply('[',2)
...
data.all %>% head()
data.all %>% names()
data.use <- data.all %>% select(1:2,25:29)


##### 전처리 데이터 저장 
#setwd("C:/Users/user/R_project/정제데이터")
write.csv(data.use,"대전시_인구수_전처리.csv",fileEncoding = "cp949",row.names = F)


##### 성별 분류X
# data.men <- data[,25:47] %>% as.data.frame()
# data.women <- data[,48:70] %>% as.data.frame()
# data.men$읍면동명 <- data.all[,1]
# data.women$읍면동명 <- data.all[,1]
# data.men <- data.men %>% relocate(읍면동명)
# data.women <- data.women %>% relocate(읍면동명)
# data.men %>% head()
